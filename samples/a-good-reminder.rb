...
Shoes.app :title => "A Gentle Reminder",
          :width => 500, :height => 560, :resizable => true do

 #@prob_count=4 if @prob_count.nil?
  ...
  stack :margin => 10, :margin_top => 50 do
    ...
    stack do
      ...
      @gui_todo= para #( "Problem 1") #, :stroke => red, :fill => yellow
    end
  ...
  def refresh_todo
#    @gui_todo.replace *( @todo.map {|item|  [item, '  '] + [link('Done') {complete_todo item}] + ['  '] +
#                          [link('Forget it') {forget_todo item}] + ["\n"]}.flatten)
    x = *(
    @todo.map {|item|  [item, '  '] +
        [(button('Done') {complete_todo item})] + ['  '] +
        [(button('Forget it') {forget_todo item})] + ["\n"]}.flatten
    )
    @gui_todo.replace (x)
#NOTE: left off the * in *(x) to show incorrect parameters and missing block.
    # This kind of bug puts me off right now from continuing with green shoes
    # and switching back to shoes4 (even thought it is tons slower on losd)
    # where I could fix the layout problems.
  end


  def refresh
    refresh_todo if @gui_todo
